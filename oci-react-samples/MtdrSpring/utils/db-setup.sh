...
CREATE TABLE TASKSTATUS (
    TaskStatusId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200)
);
CREATE TABLE TASK (
    TaskId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200),
    ...
    FOREIGN KEY (UserTeamId) REFERENCES USERTEAM(UserTeamId)
);
CREATE TABLE SPRINT (
    SprintId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
...
CREATE TABLE PROJECT (
    ProjectId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(500)
);
CREATE TABLE USER (
    UserId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Email NVARCHAR2(100),
...
CREATE TABLE USERTYPE (
    UserType NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200)
);
CREATE TABLE USERTEAM (
    UserTeamId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Foreign Key (UserId) REFERENCES USER(UserId),
    Foreign Key (TeamId) REFERENCES TEAM(TeamId)
);
CREATE TABLE TEAM (
    TeamId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200),
    FOREIGN KEY (TeamTypeId) REFERENCES TEAMTYPE(TeamTypeId)
);
CREATE TABLE TEAMTYPE (
    TeamTypeId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200)
);
CREATE TABLE TASKUPDATE (
    TaskUpdateId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    TimeStamp TIMESTAMP,
    FOREIGN KEY (UpdateTypeId) REFERENCES UPDATETYPE(UpdateTypeId),
    FOREIGN KEY (TaskId) REFERENCES TASK(TaskId),
    FOREIGN KEY (UserId) REFERENCES USER(UserId)
);
CREATE TABLE UPDATETYPE (
    UpdateTypeId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200)
);
CREATE TABLE SPRINTUPDATE (
    SprintUpdateId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    TimeStamp TIMESTAMP,
    Foreign Key (UpdateTypeId) REFERENCES UPDATETYPE(UpdateTypeId),
    Foreign Key (SprintId) REFERENCES SPRINT(SprintId),
    Foreign Key (UserId) REFERENCES USER(UserId)
...
CREATE TABLE CONVERSATION (
    ConversationId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    StartTime TIMESTAMP,
    EndTime TIMESTAMP
);
CREATE TABLE MESSAGE (
    MessageId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Content NVARCHAR2(100),
    Foreign Key (UserId) REFERENCES USER(UserId),
    Foreign Key (ConversationId) REFERENCES CONVERSATION(ConversationId)
);
CREATE TABLE BOTMENU (
    BotMenuId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Name NVARCHAR2(100),
    Description NVARCHAR2(200),
    Foreign Key (UserTypeId) REFERENCES USERTYPE(UserTypeId)
);
CREATE TABLE BOTOPTION (
    BotOptionId NUMBER GENERATED ALWAYS AS IDENTITY PRYMARY KEY,
    Text NVARCHAR2(100),
    Description NVARCHAR2(200),
